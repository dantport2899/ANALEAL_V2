...
class AccessUser
{
    ...
    function login ( $username, $password ) {
        ...
        if (empty($username)) {
            ...
            $user = $conexion->consulta("SELECT * FROM `usuarios` WHERE ".$user_login." = '".$username."'");
    ...
    function prepare_response_for_collection( $data ) {
        global $usuariosClass;

        $perfil = $usuariosClass->get_perfil($data->id);
        print_r($perfil);

        $data = array(
            'id' => (int) $data->id,
            'status' => (int) $data->estatus,
            'registered' => $data->user_registered,
            'name' => $data->nombres,
            'nicename' => $data->nombres,
            'logged' => true,
            'image' => $perfil->image,
